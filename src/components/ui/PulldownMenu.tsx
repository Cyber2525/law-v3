...
export function PulldownMenu<T extends string = string>({
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      if (isTouchInteractionRef.current && (isPointerActive || isInitialDragActive) && lastPointerPosRef.current && menuRef.current && (menuPos.top !== 0 || menuPos.right !== 0)) {
        ...
        targetDragScaleRef.current = 1.0;
        if (!isPointerActive && !isInitialDragActive) {
          initialClickPosRef.current = null;
          initialClickDistanceRef.current = null;
        }
      }
