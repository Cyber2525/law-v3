...
export const NativeToggle: React.FC<NativeToggleProps> = ({ 
  ...
  const PROXIMITY_THRESHOLD = 110; // Distance in pixels to shrink knob back into circle (2x farther)
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    const handleGlobalMove = (e: PointerEvent) => {
      ...
      // --- Proximity Logic in All Directions (Radial / 2D) ---
      if (containerRef.current) {
          const rect = containerRef.current.getBoundingClientRect();
          const centerX = rect.left + rect.width / 2;
          const centerY = rect.top + rect.height / 2;
          
          // Calculate distance in all directions (including up/down/diagonal)
          const dx = e.clientX - centerX;
          const dy = e.clientY - centerY;
          const totalDistance = Math.hypot(dx, dy);
          
          // Only expand if pointer is within radial threshold
          setIsNear(totalDistance < PROXIMITY_THRESHOLD);
      }
